...
(define (copy-closure-env-to-stack)
    (let ((copy-loop (gensym 'copy_closure))
      ...
      `((mov r8 0)
        ...
        (je ,copy-done)
        ,@assert-pair
        (xor rax ,type-pair) ;; adrress
        (mov rbx (offset rax 1)) ;; next
        (mov rax (offset rax 0)) ;; curent value
        ;;copy
        (mov (offset rcx -1) rax) ;;store value on stack
        (sub rcx 8) ;;increase stack pointer
        (mov rax rbx) ;; get the next value
        (add r8 ,add-val)
        (jmp ,copy-loop)
        ,copy-done
        (mov (offset r9 0) r8)
        ...
        ;; TODO: update to do arity checking
         (let ((i (* 32 (length xs)))
           ...
           `(,(symbol->label name)
               ...
               ;;if not empty return 
               ;;setup
               (sub rsp ,j) ;;skip the stack pointer
               (mov rcx rsp) ;;tmp pointer
               (mov r8 rdi) ;; address of the heap
               ...
               (mov r9 (offset rcx -1)) ;; get the value from the stack
               (mov (offset r8 0) r9) ;;store the value on heap

               (add r8 16)
               (mov r9 r8)
               (or r9 ,type-pair)
               
               (mov (offset r8 -1) r9) ;;pair pointer to next value

               (add rbx 32) ;; increase the value of the stack length 

               (sub rcx 8) ;; move the stack pointer
               ...
               (mov r9 r8)
               (mov r9 ,imm-type-empty)
               (mov (offset r8 0) r9) ;;set last empty
               (mov rax rdi)
               (or rax ,type-pair)
               (mov r9 rax)
